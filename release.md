...
Docker image release cycle and KhulnaSoft product
---
We consider the **docker image** as part of the KhulnaSoft **product**. Therefore, it follows the same release process.

Overview
--------

Release of a new version of the official KhulnaSoft Docker images is made of several operations:

1. bump the version of KhulnaSoft in Dockerfiles
...
3. Update the docker hub KhulnaSoft's documentation (if applicable)
4. Update Docker Hub's KhulnaSoft images
5. add a GIT tag for the new version 


Bump the version of KhulnaSoft in Dockerfiles
-----------------------------

The version of KhulnaSoft is hardcoded in each Dockerfile of this repository and must be updated in master branch.

Update the docker hub KhulnaSoft's documentation (if applicable)
-------------------------------
...
1. The markdown format must follow a certain standard, otherwise automated tests will fail. You can test with the `markdownfmt.sh` tool included in the repository, for example `./markdownfmt.sh -d khulnasoft/content.md` will output the diff that would have to be done to make the tests pass. You can use the `patch` command to apply the changes, for example: `./markdownfmt.sh -d khulnasoft/content.md | patch khulnasoft/content.md`
2. Verify the Pull Request passes the automated tests (visible in the status of the PR)

To control the generated content of the Docker Hub page, look around in the files in `.template-helpers` of the [`docs` repository][docs]. For example, the "Where to get help" section is customized by a copy of `.template-helpers/get-help.md` in `khulnasoft/get-help.md`.

Until KhulnaSoft is released and the public artifacts are available, keep your PR a draft PR to make it clear it is not ready to be merged yet.

For more and up to date documentation, see https://github.com/docker-library/docs.


Update Docker Hub's KhulnaSoft images
-----------------------
