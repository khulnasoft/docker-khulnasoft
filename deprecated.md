...
KhulnaSoft Docker images are dogfooded.

This implies that Docker images are produced for every artifact produced from the dogfood branch of KhulnaSoft (see the [dogfood_docker_build_task](https://github.com/KhulnaSoft/khulnasoft-enterprise/blob/master/.cirrus.yml#L263)).

Docker images of KhulnaSoft are built from publicly available artifacts, which means the Dockerfile can simply download them from the public place they are hosted (`binaries.khulnasoft.com`).
...
This artifact should be named `khulnasoft-${KHULNASOFT_VERSION}.zip` and located in the `zip` directory of the `docker build` context. If such file exists, this artifact will be used to build the image instead of downloading it from `binaries.khulnasoft.com`.
...
Currently, ITs are "simply" building a given image of KhulnaSoft and make sure they can run it and have KhulnaSoft responding on HTTP calls in a reasonable time.
