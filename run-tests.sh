...
wait_for_khulnasoft() {
    local image=$1 i web_up=no khulnasoft_up=no
    ...
    for ((i = 0; i < 20; i++)); do
        info "$image: waiting for khulnasoft to be ready ..."
...
wait_for_khulnasoft_dce() {
    local image=$1-app i web_up=no khulnasoft_up=no
...
sanity_check_image() {
    ...
    if [[ $2 == docker ]]; then
        ...
        if wait_for_khulnasoft "$image"; then
            info "$image: OK !"
            result=ok
        else
        ...
        export PORT=$port
        export IMAGE=$1
        docker compose up -d khulnasoft
        if wait_for_khulnasoft_dce "$image"; then
            info "$image-app: OK !"
            result=ok
        else
