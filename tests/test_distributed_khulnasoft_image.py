...
class TestDockerKhulnasoft(Executor):
    ...
    def test_compose_3idx1cm_custom_repl_factor(self):
        ...
        try:
            ...
            for container in container_mapping:
                ...
                try:
                    assert inventory_json["khulnasoft_indexer"]["hosts"] == ["idx1", "idx2", "idx3"]
                    assert inventory_json["khulnasoft_cluster_master"]["hosts"] == ["cm1"]
            ...
            for container in containers:
                ...
                if container_name == "cm1":
                    ...
                    url = "https://localhost:{}/services/cluster/config/config?output_mode=json".format(khulnasoftd_port)
    ...
    def test_compose_1idx3sh1cm1dep(self):
        ...
        try:
            ...
            for container in container_mapping:
                ...
                self.check_common_keys(inventory_json, container_mapping[container])
                try:
                    assert inventory_json["khulnasoft_indexer"]["hosts"] == ["idx1"]
                    assert inventory_json["khulnasoft_search_head_captain"]["hosts"] == ["sh1"]
                    assert inventory_json["khulnasoft_search_head"]["hosts"] == ["sh2", "sh3"]
                    assert inventory_json["khulnasoft_cluster_master"]["hosts"] == ["cm1"]
                    assert inventory_json["khulnasoft_deployer"]["hosts"] == ["dep1"]
                except KeyError as e:
                    self.logger.error(e)
                    raise e
            # Check Khulnasoftd on all the containers
            ...
            for container in containers:
                ...
                if container_name == "sh1":
                    url = "https://localhost:{}/servicesNS/nobody/system/configs/conf-server/shclustering?output_mode=json".format(khulnasoftd_port)
                    kwargs = {"auth": ("admin", self.password), "verify": False}
                    status, content = self.handle_request_retry("GET", url, kwargs)
    ...
    def test_compose_1uf1so(self):
        ...
        for container in container_mapping:
            ...
            self.check_common_keys(inventory_json, container_mapping[container])
            try:
                assert inventory_json["khulnasoft_standalone"]["hosts"] == ["so1"]
            except KeyError as e:
                self.logger.error(e)
                raise e
        # Search results won't return the correct results immediately :(
    ...
    def test_compose_1so1cm_connected(self):
        ...
        for container in containers:
            ...
            khulnasoftd_port = self.client.port(container["Id"], 8089)[0]["HostPort"]
            if container_name == "cm1":
                status, content = self.handle_request_retry("GET", "https://localhost:{}/services/cluster/master/searchheads?output_mode=json".format(khulnasoftd_port), 
    ...
    def test_adhoc_1cm_idxc_pass4symmkey(self):
        ...
        try:
            self.project_name = self.generate_random_string()
            cid = self.client.create_container(self.KHULNASOFT_IMAGE_NAME, tty=True, ports=[8089], name=self.project_name,
                                            environment={
                                                            "DEBUG": "true", 
                                                            ...
                                                            "KHULNASOFT_IDXC_PASS4SYMMKEY": "keepsummerbeingliketotallystokedaboutlikethegeneralvibeandstuff",
                                                            "KHULNASOFT_IDXC_LABEL": "keepsummersafe",
                                                        },
    ...
    index:
        ...
        try:
            ...
            exec_command = self.client.exec_create(cid, "cat /opt/khulnasoft/etc/master-apps/_cluster/local/indexes.conf", user="khulnasoft")
            std_out = self.client.exec_start(exec_command)
    ...
    def test_compose_2idx2sh(self):
        ...
        for container in container_mapping:
            ...
            self.check_common_keys(inventory_json, container_mapping[container])
            try:
                assert inventory_json["khulnasoft_indexer"]["hosts"] == ["idx1", "idx2"]
                assert inventory_json["khulnasoft_search_head"]["hosts"] == ["sh1", "sh2"]
            except KeyError as e:
                self.logger.error(e)
                raise e
        # Check Khulnasoftd on all the containers
        ...
        for container in containers:
            ...
            if "sh1" in c_name or "sh2" in c_name:
                khulnasoftd_port = self.client.port(container["Id"], 8089)[0]["HostPort"]
                url = "https://localhost:{}/services/search/distributed/peers?output_mode=json".format(khulnasoftd_port)
                kwargs = {"auth": ("admin", self.password), "verify": False}
                status, content = self.handle_request_retry("GET", url, kwargs)
                assert status == 200
                output = json.loads(content)
    ...
    def test_compose_2idx2sh1cm(self):
        ...
        for container in containers:
            ...
            if container_name == "cm1":
                ...
                status, content = self.handle_request_retry("GET", "https://localhost:{}/services/cluster/master/peers?output_mode=json".format(khulnasoftd_port), 
                                                            {"auth": ("admin", self.password), "verify": False})
                assert status == 200
                output = json.loads(content)
  ...
  conf:
        ...
        try:
            ...
            for container in containers:
                ...
                if container_name == "depserver1":
                    ...
                    exec_command = self.client.exec_create(container["Id"], "ls /opt/khulnasoft/etc/apps/khulnasoft_app_example/local/", user="khulnasoft")
                    std_out = self.client.exec_start(exec_command)
                    assert "savedsearches.conf" in std_out
                    assert "app.conf" in std_out
                    exec_command = self.client.exec_create(container["Id"], "cat /opt/khulnasoft/etc/apps/khulnasoft_app_example/local/app.conf", user="khulnasoft")
                    std_out = self.client.exec_start(exec_command)
                    assert "state = disabled" in std_out
                    # Check that the app exists in etc/deployment-apps
                    exec_command = self.client.exec_create(container["Id"], "ls /opt/khulnasoft/etc/deployment-apps/khulnasoft_app_example/local/", user="khulnasoft")
                    std_out = self.client.exec_start(exec_command)
                    assert "savedsearches.conf" in std_out
                    assert "app.conf" in std_out
                    exec_command = self.client.exec_create(container["Id"], "cat /opt/khulnasoft/etc/deployment-apps/khulnasoft_app_example/local/app.conf", user="khulnasoft")
                    std_out = self.client.exec_start(exec_command)
                    assert "# Autogenerated file " == std_out
                if container_name == "cm1":
                    # Check if the created file exists
                    exec_command = self.client.exec_create(container["Id"], "cat /opt/khulnasoft/etc/users/admin/user-prefs/local/user-prefs.conf", user="khulnasoft")
                    std_out = self.client.exec_start(exec_command)
                ...
                if container_name == "uf1":
                    ...
                    for i in range(RETRIES):
                        try:
                            ...
                            assert status == 200
                            assert json.loads(content)["entry"][0]["content"]["version"] == "0.0.1"
                            exec_command = self.client.exec_create(container["Id"], "ls /opt/khulnasoftforwarder/etc/apps/khulnasoft_app_example/local/", user="khulnasoft")
                            std_out = self.client.exec_start(exec_command)
                            assert "savedsearches.conf" in std_out
                            assert "app.conf" in std_out
                            exec_command = self.client.exec_create(container["Id"], "cat /opt/khulnasoftforwarder/etc/apps/khulnasoft_app_example/local/app.conf", user="khulnasoft")
                            std_out = self.client.exec_start(exec_command)
                            assert "# Autogenerated file" in std_out
                            assert "state = enabled" in std_out
                        except Exception as e:
    ...
    def test_compose_3idx1cm_khulnasofttcp_ssl(self):
        self.project_name = self.generate_random_string()
        # Generate default.yml
        cid = self.client.create_container(self.KHULNASOFT_IMAGE_NAME, tty=True, command="create-defaults")
        self.client.start(cid.get("Id"))
    ...
    ssl: true'''.format(passphrase), output, flags=re.DOTALL)
        ...
        try:
            ...
            for container in containers:
                ...
                if container_name == "cm1":
                    exec_command = self.client.exec_create(cid, "cat /opt/khulnasoft/etc/system/local/outputs.conf", user="khulnasoft")
                    std_out = self.client.exec_start(exec_command)
